...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_concept.png)](/nativesso)
...
4. Device Secret
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_spec_overview_0.png)](/nativesso)
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_spec_overview_1.png)](/nativesso)
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_spec_overview_2.png)](/nativesso)
In response, a token exchange response containing a new set of tokens for App 2
is returned.
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_spec_overview_3.png)](/nativesso)
To summarize, the following diagram illustrates the overview.
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_spec_overview.png)](/nativesso)
### Device Secret
...
`
This example actually works. When the authorization page is displayed, please
enter `inga` and `inga` as the login ID and password,
respectively. If you are already logged in and would like to display the login
ID field and password field again, add `&prompt=login`
...
client_id=app_1&grant_type=authorization_code&code=\`}&redirect_uri=https://nextdev-api.authlete.net/api/mock/redirection
`
If you try this example in practice, replace `\`}`
with the actual value (the authorization code obtained as a result of the
...
For an explanation of RFC 8693, please refer to the article
"[RFC 8693 OAuth
2.0 Token Exchange](https://www.authlete.com/developers/token_exchange/)".
...
The following is an example of a token exchange request by a public client:
  `POST https://trial.authlete.net/api/token HTTP/1.1
Content-Type: application/x-www-form-urlencoded
client_id=app_2&grant_type=urn:ietf:params:oauth:grant-type:token-exchange&audience=https://trial.authlete.net&subject_token=\`}&subject_token_type=urn:ietf:params:oauth:token-type:id_token&actor_token=\`}&actor_token_type=urn:openid:params:token-type:device-secret&scope=openid
`
If you want to try this example in practice, replace `\`}`
...
3. Go to the `Native SSO` section and toggle `Allow` to enable.
...
3. Go to the `Supported Scopes` and click the `Add` button.
4. Enter `device_sso` for the scope name and click add.
5. Once back in the scope page, click `Save Changes` to apply the update.
...
2. Go to `Supported Grant Types`, and select the `Token_Exchange` option.
...
To register  the `device_sso` scope:
  1. Login to the [Authlete Management Console](https://console.authlete.com/)
2. Navigate to `Client Settings > Tokens and Claims > Advanced > Scope`
3. Go to the `Add Requestable scope` and enter  `device_sso`for the scope name.
4. Once back in the scope page, click `Save Changes` to apply the update.
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_token_management.png)](/nativesso)
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_authorization_endpoint.png)](/nativesso)
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_token_endpoint.png)](/nativesso)
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_session_id_in_exchange.png)](/nativesso)
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_device_secret_in_code_or_refresh.png)](/nativesso)
...
[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_device_secret_in_exchange.png)](/nativesso)
...
```[![](https://storage.googleapis.com/authlete-website/img/developers/nativesso/nativesso_api.png)](/nativesso)
...
Below are some tips for reading through the sample implementation:
  - The session ID passed to the `/auth/authorization/issue` API is obtained in the [AuthorizationDecisionEndpoint](https://github.com/authlete/java-oauth-server/blob/master/src/main/java/com/authlete/jaxrs/server/api/AuthorizationDecisionEndpoint.java) by executing `HttpServletRequest.getSession(false).getId()`. However, since this session is the HTTP session between the web server and browser, a different mechanism would likely be used in a production-ready Native SSO implementation.
- The obtained session ID is passed to the [AuthorizationDecisionHandler](https://github.com/authlete/authlete-java-jaxrs/blob/master/src/main/java/com/authlete/jaxrs/AuthorizationDecisionHandler.java) via the [AuthorizationDecisionHandlerSpi](https://github.com/authlete/authlete-java-jaxrs/blob/master/src/main/java/com/authlete/jaxrs/spi/AuthorizationDecisionHandlerSpi.java) interface.
- The AuthorizationDecisionHandler calls the Authlete `/auth/authorization/issue` API through the `callAuthorizationIssue` method of [AuthleteApiCaller](https://github.com/authlete/authlete-java-jaxrs/blob/master/src/main/java/com/authlete/jaxrs/AuthleteApiCaller.java).
- The dispatch logic based on the `action` in the `/auth/token` API response is written in the [TokenRequestHandler](https://github.com/authlete/authlete-java-jaxrs/blob/master/src/main/java/com/authlete/jaxrs/TokenRequestHandler.java).
- If the `action` is `NATIVE_SSO`, TokenRequestHandler invokes the `nativeSso` method of the [TokenRequestHandlerSpi](https://github.com/authlete/authlete-java-jaxrs/blob/master/src/main/java/com/authlete/jaxrs/spi/TokenRequestHandlerSpi.java) interface.
- The implementation of TokenRequestHandlerSpi, [TokenRequestHandlerSpiImpl](https://github.com/authlete/java-oauth-server/blob/master/src/main/java/com/authlete/jaxrs/server/api/TokenRequestHandlerSpiImpl.java), calls the `process` method of the [NativeSsoProcessor](https://github.com/authlete/java-oauth-server/blob/master/src/main/java/com/authlete/jaxrs/server/api/NativeSsoProcessor.java) from within its `nativeSso` method.
- The NativeSsoProcessor extracts the session ID, device secret, and device secret hash from the instance of the [TokenResponse](https://github.com/authlete/authlete-java-common/blob/master/src/main/java/com/authlete/common/dto/TokenResponse.java) class, which represents the response from the `/auth/token` API.
...
- The check for whether a session ID is valid is done by calling the `isActiveSessionId(String)` method of the [SessionTracker](https://github.com/authlete/java-oauth-server/blob/master/src/main/java/com/authlete/jaxrs/server/core/SessionTracker.java). SessionTracker implements the HttpSessionListener interface and monitors session creation and deletion. It is registered as a listener in [web.xml](https://github.com/authlete/java-oauth-server/blob/master/src/main/webapp/WEB-INF/web.xml).
- The relationship between device secret, session ID, device secret hash, and device identifier is represented by the [DeviceSecret](https://github.com/authlete/java-oauth-server/blob/master/src/main/java/com/authlete/jaxrs/server/nativesso/DeviceSecret.java) class.
- Instances of DeviceSecret are managed by the [DeviceSecretManager](https://github.com/authlete/java-oauth-server/blob/master/src/main/java/com/authlete/jaxrs/server/nativesso/DeviceSecretManager.java).
  ...
  2. The `requested_token_type` parameter is specified, and its value is a known token type.
  3. The `subject_token_type` parameter is specified, and its value is `urn:ietf:params:oauth:token-type:id_token`.
  4. The `subject_token` parameter is specified, and its value (ID token) passes all the following validation checks:
...
- The signature verification is successful.
  5. The `actor_token` parameter is specified.
  In draft 07 of the Native SSO specification, it is stipulated that if the
  `scope` parameter is included in the token exchange request,
  the scope list must include `openid`.
  However, due to the reasons described in [AB/Connect ISSUE 2178:
  Native SSO the openid scope on token exchange](https://bitbucket.org/openid/connect/issues/2178), Authlete’s current
  implementation does not check whether `openid` is included.
  ## References
...
- Documentation: [RFC 8693 OAuth 2.0 Token Exchange](https://www.authlete.com/ja/developers/token_exchange/)
  #### On this page
- [Concept](#concept)
- [Specification Overview](#spec_overview)
  [Device Secret](#device_secret)
- [Specification Details](#spec_details)
  [App 1 Authorization Request](#app1_authorization_request)
  [App 1 Token Request](#app1_token_request)
  [App 1 Token Response](#app1_token_response)
  [App 2 Token Request](#app2_token_request)
  [App 2 Token Response](#app2_token_response)
- [Settings](#settings)
  [Service Settings](#service_settings)
  [nativeSsoSupported Property](#service_property)
  [device_sso Scope](#service_scope)
  [Service Grant Type](#service_grant_type)
  [Console Configuration](#console-configuration)
  [Client Settings](#client_settings)
  [device_sso Scope](#client_scope)
  [Client Grant Type](#client_grant_type)
  [Console Configuration](#console-configuration-1)
- [OpenID Provider Implementation](#openid_provider_implementation)
  [Authentication Session and Device Secret Management](#token_management)
  [Authorization Endpoint Implementation](#authorization_endpoint)
  [Token Endpoint Implementation (Verification)](#token_endpoint_verification)
  [Session ID Verification](#session_id_verification)
  [Device Secret Verification (in Authorization Code and Refresh Token Flows)](#device_secret_verification_in_code_or_refresh)
  [Device Secret Verification (in Token Exchange Flow)](#device_secret_verification_in_exchange)
  [Token Endpoint Implementation (nativesso API call)](#token_endpoint_nativesso_api_call)
  [nativesso Request](#nativesso_request)
  [nativesso Response](#nativesso_response)
  [Single Logout](#single_logout)
  [Sample Implementation](#sample_implementation)
- [Authlete Implementation](#authlete_implementation)
  [Authlete Version](#authlete_version)
  [Native SSO Version](#nativesso_version)
  [Token Exchange Verification](#token_exchange_verification)
