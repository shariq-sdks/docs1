...
description: 'Learn the fundamentals of OAuth 2.0 authorization framework, including authorization flows, grant types, and security concepts.'
---
...
1. [Authlete] Complete steps in [Getting Started](/developers/getting_started/)
1. [Lambda] Renew `GetHelloWorld` lambda functionCreate a [Deployment Package](http://docs.aws.amazon.com/lambda/latest/dg/nodejs-create-deployment-pkg.html) and upload it as the implementation of `GetHelloWorld` lambda function.
`$ mkdir gethelloworld
$ cd gethelloworld
...
1. [Lambda] Edit the timeout valueIncrease the timeout value of the lambda function (e.g. to 30 sec).
1. [API Gateway] [Method Request] Add `access_token` as a query parameter to `GET /mydemoresource`
1. [API Gateway] [Integration Request] Set a template as is shown below
`{ "access_token": "$input.params('access_token')" }
`
1. [Amazon API Gateway] [Method Response] Add 400, 401, 403 and 500 as HTTP status codes
1. [Amazon API Gateway] [Integration Response] Set mappings as shown below
...
[API Gateway] Complete steps in [Getting Started with Amazon API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/getting-started.html)

[Authlete] Complete steps in [Getting Started](/developers/getting_started/)

[Lambda] Renew `GetHelloWorld` lambda functionCreate a [Deployment Package](http://docs.aws.amazon.com/lambda/latest/dg/nodejs-create-deployment-pkg.html) and upload it as the implementation of `GetHelloWorld` lambda function.
...
[Lambda] Edit the timeout valueIncrease the timeout value of the lambda function (e.g. to 30 sec).

[API Gateway] [Method Request] Add `access_token` as a query parameter to `GET /mydemoresource`
...
1. Access `/mydemoresource` endpoint with an access token
...
Access `/mydemoresource` endpoint with an access token
Access the URL below with your browser.
Don’t forget to replace your-api-id, region-id and your-access-token with your own.
