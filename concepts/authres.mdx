...
-d '{"parameters":"client_assertion_type=urn%3Aietf%3Aparams%3Aoauth%3Aclient-assertion-type%3Ajwt-bearer&client_assertion=eyJraWQi...&client_id={Client ID\`}&response_type=code&redirect_uri=https%3A%2F%2Fclient.example.com%2Fcb&scope=myscope&code_challenge=E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM&code_challenge_method=S256&response_mode=jwt"}'
```
...
{\`
  ...
  "responseContent": "{\"expires_in\":599,\"request_uri\":\"urn:ietf:params:oauth:request_uri:6uOtgZjBYPPspJLf630c8iJfi4xql_G3R7MKMjgTxRk\"\`}"
}
...
-u '{\`Service API Key}:{Service API Secret\`}' \
-H 'Content-type: application/json' \
-d '{\`"ticket":"4rmnZ-mY...","subject":"john","result":"AUTHORIZED"}'
```
