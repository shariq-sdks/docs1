---
title: "RFC 9470 OAuth 2.0 Step Up Authentication Challenge Protocol - Authlete"
...
3. RFC 9470 OAuth 2.0 Step Up Authentication Challenge Protocol
# Overview
...
```shell
curl -v -X POST https://.authlete.com/api//auth/authorization \
-H 'Content-Type: application/json' \
-u 'Authorization: Bearer Service Access Token e.g. Xg6jVpJCvsaXvy2ks8R5WzjdMYlvQqOym3slDX0wNhQ>' \ --data-urlencode parameters="response_type=code&scope=openid&client_id=$&redirect_uri=$&state=$&claims=\"]\`}}}" >\`} authorization_response.json
```
The value of `$` in the command line above needs to be registered in advance as one of "Supported Authentication Context Class References" (which corresponds to the `acr_values_supported` server metadata). Otherwise, the specified ACR would be ignored.
...
$ AUTH_TIME=`date +%s`
$ curl -v -X POST https://.authlete.com/api//auth/authorization/issue \
-H 'Content-Type: application/json' \
...
$ curl -v -X POST https://.authlete.com/api//auth/token \
-H 'Content-Type: application/json' \
-u 'Authorization: Bearer Service Access Token e.g. Xg6jVpJCvsaXvy2ks8R5WzjdMYlvQqOym3slDX0wNhQ>' \
--data-urlencode parameters="client_id=$&grant_type=authorization_code&code=$&redirect_uri=$ -d clientId=$ -d clientSecret=$ >\`} token_response.json
```
...
```shell
curl -v -X POST https://.authlete.com/api//auth/authorization/fail \
-H 'Content-Type: application/json' \
-u 'Authorization: Bearer Service Access Token e.g. Xg6jVpJCvsaXvy2ks8R5WzjdMYlvQqOym3slDX0wNhQ>' \
...
$ curl -v -X POST https://.authlete.com/api//auth/introspection/standard \
-H "Content-Type:application/json" \
-u 'Authorization: Bearer Service Access Token e.g. Xg6jVpJCvsaXvy2ks8R5WzjdMYlvQqOym3slDX0wNhQ>' \
--data-urlencode parameters="token=$ >\`} introspection_standard_response.json
...
```shell
$ curl -v -X POST https://.authlete.com/api//auth/introspection \
-H 'Content-Type:application/json' \
-u 'Authorization: Bearer Service Access Token e.g. Xg6jVpJCvsaXvy2ks8R5WzjdMYlvQqOym3slDX0wNhQ>'  \
...
```shell
$ curl -v -X POST https://.authlete.com/api/{serviceId}/auth/introspection \
-H 'Content-Type:application/json' \
-u 'Authorization: Bearer Service Access Token e.g. Xg6jVpJCvsaXvy2ks8R5WzjdMYlvQqOym3slDX0wNhQ>'  \
