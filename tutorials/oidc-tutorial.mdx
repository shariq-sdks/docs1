...
# Environment setup
Note
Settings in this section are the same as described in Authlete API Tutorial - [OAuth 2.0 Basics](/tutorial). If you've already completed that tutorial, you can skip this section.
Consult instructions “[Sign-up to Authlete and Creating a Service](/tutorial/signup)“ for how to create an Authlete API service and register a client to the service.
...
However, in the Authlete architecture the authorization server is simply a proxy for the actual authorization logic contained in the Authlete API which is surfaced at the [/auth/authorization](https://docs.authlete.com/#auth-authorization-api) endpoint. Once received, the Authlete API does the evaluation process on the authorization server’s behalf.
...
The authorization server makes a request to Authlete’s [/auth/authorization/issue](https://docs.authlete.com/#auth-authorization-issue-api) in order to issue an authorization code. It specifies values of `subject` and `ticket` that were a part of the response from `/auth/authorization` API, as request parameters.
...
Once again, let’s simulate this step using Authlete’s  [/auth/token](https://docs.authlete.com/#auth-token-api) API to evaluate the request and create the response.
...
Let’s make the same authorization request as the previous one (using the same `nonce` value for convenience) to Authlete’s [/auth/authorization](https://docs.authlete.com/#auth-authorization-api) API (step #5).
...
```## Additional claims
Let’s make a request to Authlete’s [/auth/authorization/issue](https://docs.authlete.com/#auth-authorization-issue-api) for issuance of an authorization code. Make sure to replace `<Service ID>`, `<Service Access Token>` and `<Ticket>` by your own values generated in the previous step.
...
&redirect_uri=https://client.example.org/cb/example.com
```The authorization server is supposed to make a request to Authlete’s [/auth/token](https://docs.authlete.com/#auth-token-api) API.
```
curl -s -X POST https://us.authlete.com/api/<Service ID e.g. 10738933707579>/auth/token \
-H 'Authorization: Bearer <Service Access Token e.g. Xg6jVpJCvsaXvy2ks8R5WzjdMYlvQqOym3slDX0wNhQ>' \
-H 'Content-Type: application/json' \
...
- [/auth/authorization/fail](https://docs.authlete.com/#auth-authorization-fail-api) API (See [Generating error response with “fail” API](/kb/oauth-and-openid-connect/error-handling/fail-api/))
- [/auth/userinfo](http://docs.authlete.com/#userinfo) API (See [Access token verification in Userinfo API](/kb/oauth-and-openid-connect/userinfo-endpoint/access-token-verification-in-userinfo-ep/))
...
- [Authorization server implementations](https://github.com/authlete) (See [Authorization Server Implementation - Getting Started](/getting_started))
...
- [User authentication and confirmation of sharing authentication result](#user-authentication-and-confirmation-of-sharing-authentication-result)
- [Issuing an authorization code](#issuing-an-authorization-code)
- [Failure to issue an authorization code](#failure-to-issue-an-authorization-code)
- [Token request](#token-request)
- [Decoding ID token](#decoding-id-token)
- [Issuer identifier](#issuer-identifier)
- [Authorization Request](#authorization-request)
- [Additional claims](#additional-claims)
- [Token request](#token-request-1)
